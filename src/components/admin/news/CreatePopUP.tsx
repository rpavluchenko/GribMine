...
export default function CreatePopUp({ isOpen, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (formCreateData.image_url) {
        uploadedImageUrl = await uploadImageToServer(formCreateData.image_url)
      }

      createNews(
        {
          author: formCreateData.author,
          title: formCreateData.title,
          content: formCreateData.content.replace(/\n/g, '<br />'),
          sub_title: formCreateData.sub_title,
            ...
            <div className="relative w-full" key={name}>
              ...
              {name === 'content' ? (
                <textarea
                  ...
                  rows={4}
                  value={
                    formCreateData[name as keyof typeof formCreateData] || ''
                  }
                />
              ) : name === 'image_url' ? (
